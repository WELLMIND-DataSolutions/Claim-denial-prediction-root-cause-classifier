...
/* Top header banner */
.top-header {
    background: linear-gradient(135deg, rgba(37,99,235,0.12) 0%, rgba(13,148,136,0.12) 100%) !important;
...
.driver-name { font-family: 'Fira Code', monospace !important; font-size: 12px !important; color: #CBD5E1 !important; min-width: 190px !important; }
.driver-bar-wrap { flex: 1; height: 8px; background: rgba(255,255,255,0.05); border-radius: 6px; overflow: hidden; }
...
.driver-val { font-family: 'Fira Code', monospace !important; font-size: 12px !important; color: #94A3B8 !important; min-width: 60px !important; text-align: right !important; }

/* Cause card */
.cause-card {
    background: linear-gradient(135deg, rgba(30,58,138,0.2) 0%, rgba(6,78,59,0.2) 100%) !important;
    border: 1px solid rgba(59,130,246,0.2) !important;
    border-radius: 16px !important; padding: 24px !important;
    margin-bottom: 16px !important; box-shadow: 0 8px 32px 0 rgba(0,0,0,0.25) !important;
}
.cause-label { font-size: 18px !important; font-weight: 700 !important; color: #60A5FA !important; margin-bottom: 4px !important; }
.cause-conf { font-size: 13px !important; color: #93C5FD !important; font-weight: 500 !important; }
.fix-label { font-size: 11px !important; font-weight: 700 !important; color: #34D399 !important; text-transform: uppercase !important; letter-spacing: 0.08em !important; }
.fix-text { font-size: 14px !important; color: #A7F3D0 !important; margin-top: 6px !important; line-height: 1.6 !important; }
...
/* Form section labels */
.form-section {
    font-size: 12px !important; font-weight: 700 !important; color: #60A5FA !important;
    text-transform: uppercase !important; letter-spacing: 0.1em !important;
    margin: 28px 0 12px 0 !important;
    border-bottom: 1px solid rgba(59,130,246,0.15) !important; padding-bottom: 6px !important;
}
...
[data-testid="stTextArea"] textarea {
    background-color: #111827 !important; color: #F1F5F9 !important;
    border: 1.5px solid #1E293B !important; border-radius: 10px !important; font-size: 14px !important;
}
...
label[data-testid="stWidgetLabel"] p { color: #94A3B8 !important; font-weight: 500 !important; font-size: 13px !important; }
...
.ready-icon {
    ...
    display: flex; align-items: center; justify-content: center;
    margin: 0 auto 18px auto; font-size: 26px; color: #38BDF8;
}
.ready-title { font-size: 19px; font-weight: 700; color: #F8FAFC; margin-bottom: 8px; letter-spacing: -0.02em; }
.ready-subtitle { font-size: 13px; color: #64748B; line-height: 1.6; margin-bottom: 28px; }
.ready-subtitle strong { color: #94A3B8; }
...
.fi-blue  { background: rgba(59,130,246,0.12); border: 1px solid rgba(59,130,246,0.15); color: #38BDF8; }
.fi-amber { background: rgba(251,191,36,0.10); border: 1px solid rgba(251,191,36,0.15); color: #FBBF24; }
.fi-green { background: rgba(52,211,153,0.10); border: 1px solid rgba(52,211,153,0.15); color: #34D399; }
.fi-red   { background: rgba(248,113,113,0.10); border: 1px solid rgba(248,113,113,0.15); color: #F87171; }
.feature-title { font-size: 14px; font-weight: 600; color: #F1F5F9; }
...
status_text    = "<span style='color:#34D399;'>API Connected</span>" if ok else "<span style='color:#F87171;'>API Offline</span>"
...
with col_result:
    if submitted:
        ...
        if result:
            ...
            if risk:
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    title={"text": "Denial Probability", "font": {"size": 13, "color": "#94A3B8", "family": "Plus Jakarta Sans, sans-serif"}},
                ))
                fig.update_layout(
                    height=240,
                ...
                <div class="cause-card">
                    <div style="display:flex; align-items:center; gap:14px; margin-bottom:14px;">
                        <div style="width:38px; height:38px; border-radius:10px; flex-shrink:0;
                            ...
                            display:flex; align-items:center; justify-content:center;
                            font-size:20px; color:#60A5FA;">&#9670;</div>
                        <div>
                            <div class="cause-label">{cause['display_name']}</div>
                            <div class="cause-conf">Confidence: {cause['confidence']:.1%}</div>
                        </div>
                    </div>
                    <div style="padding-top:14px; border-top:1px solid rgba(255,255,255,0.05);">
                    ...
                    <div class="pill-row" style="border-top:1px solid rgba(255,255,255,0.04); padding-top:12px; margin-top:14px;">
        ...
        st.markdown("""
<style>
.ready-card { background: rgba(21,29,48,0.45); backdrop-filter: blur(16px);
    border: 1px solid rgba(255,255,255,0.05); border-radius: 16px;
    padding: 36px 24px; box-shadow: 0 8px 32px 0 rgba(0,0,0,0.3);
    margin-top: 24px; text-align: center; }
.ready-icon { width: 52px; height: 52px; border-radius: 14px;
    background: linear-gradient(135deg,rgba(37,99,235,0.15),rgba(13,148,136,0.15));
    border: 1.5px solid rgba(59,130,246,0.2);
    display: flex; align-items: center; justify-content: center;
    margin: 0 auto 18px auto; font-size: 26px; color: #38BDF8; }
...
.ready-subtitle strong { color: #94A3B8; }
.feature-list { display: flex; flex-direction: column; gap: 10px; text-align: left; }
.feature-item { display: flex; align-items: center; gap: 14px; padding: 14px 16px;
    background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; }
.feature-icon { width: 38px; height: 38px; border-radius: 10px; flex-shrink: 0;
    display: flex; align-items: center; justify-content: center; font-size: 20px; }
.fi-blue  { background: rgba(59,130,246,0.12); border: 1px solid rgba(59,130,246,0.15); color: #38BDF8; }
.fi-amber { background: rgba(251,191,36,0.10); border: 1px solid rgba(251,191,36,0.15); color: #FBBF24; }
.fi-green { background: rgba(52,211,153,0.10); border: 1px solid rgba(52,211,153,0.15); color: #34D399; }
